...
def play(board, moves):
    ...
    for pos in range(len(board)):                   # for every spot on the board
        if board[pos]:                              # if it has a peg
            for over, to in allowedMoves[pos]:      # loop over all allowed moves from that position
...
if __name__ == "__main__":
    uniqueStartingPositions = [0, 1, 3, 4]    # all other positions are rotations or mirrors of these
